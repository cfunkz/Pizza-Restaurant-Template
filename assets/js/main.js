/*──────────────────────────────────────────────────────────────────────────────
  Page Loader — hides once fonts + all resources are ready
──────────────────────────────────────────────────────────────────────────────*/
(function () {
  let dismissed = false;
  function hideLoader() {
  ...
  setTimeout(hideLoader, 4000); // safety cap
}());
    ...
    cart.items.forEach((item, idx) => {
      ...
      const extrasHtml = (item.extraGroups && item.extraGroups.length)
        ? item.extraGroups.map(g =>
            `<div class="cart-extra-group">
              <span class="cart-extra-label">${g.label}:</span>
              ${g.items.map(i => `<span class="cart-extra-tag">${i}</span>`).join('')}
            </div>`
          ).join('')
        : '';
      ...
      $li.html(`
        <img src="${item.image}" alt="${item.name}" width="88" height="88"
          class="rounded" style="object-fit:cover;flex-shrink:0;align-self:flex-start;margin-top:2px"/>
