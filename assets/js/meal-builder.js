...
$(function () {
  ...
  $('#add-meal-btn').on('click', function () {
    ...
    const extraGroups = [
      ...
      { label: 'Sides',    items: Object.entries(state.sides).filter(([, q])    => q > 0).map(([n, q]) => (q > 1 ? `${q}× ` : '') + n) },
      { label: 'Drinks',   items: Object.entries(state.drinks).filter(([, q])   => q > 0).map(([n, q]) => (q > 1 ? `${q}× ` : '') + n) },
    ...
    cart.items.push({
      id:          'meal-' + Date.now(),
      name:        `${state.base.name} (${state.size})`,
